...
- **新增ZMQ服务器增强**：新增--once参数支持消息播放后的优雅操作
...
- **zmq_events_20260519_181735.jsonl**：包含动作事件和得分事件，涵盖完整的比赛过程
- **zmq_events_20260519_182140.jsonl**：包含动作事件和得分事件，提供不同的时间线
- **zmq_events_20260519_144636.jsonl**：包含动作事件和得分事件，提供早期的比赛数据
- **zmq_events_20260520_134352.jsonl**：新增的事件文件，提供更完整的数据集
- **zmq_events_20260520_135655.jsonl**：新增的事件文件，提供不同的时间戳数据
...
- [zmq_events_20260519_181735.jsonl:1-69](file://zmq_events_20260519_181735.jsonl#L1-L69)
- [zmq_events_20260519_182140.jsonl:1-68](file://zmq_events_20260519_182140.jsonl#L1-L68)
...
**新增的ZMQ事件流JSONL文件**进一步丰富了系统的事件处理能力，提供了更丰富的动作和得分事件数据，支持更复杂的实时事件驱动应用。多个时间戳的事件文件为系统提供了更全面的数据支持，包括早期数据、完整数据和不同时间线的数据。
...
该系统为构建高质量的实时语音应用和事件驱动应用提供了坚实的基础，可以进一步扩展以支持更多的音频格式、优化识别模型、增强事件处理能力和完善监控功能。新增的字幕播放器功能、ZMQ事件流处理和--once参数功能特别适用于视频内容的实时字幕生成和播放场景，为多媒体应用开发提供了完整的解决方案。

**WebSocket服务实现状态变更**意味着项目的WebSocket功能得到了进一步的完善和优化，为用户提供更加稳定和高效的实时语音处理体验。结合ZMQ服务器增强功能，Vue3Speech项目为现代多媒体应用开发提供了强大而灵活的技术基础。