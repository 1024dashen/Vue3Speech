...
Vue3 Speech 是一个基于 Vue3 和 FastAPI 构建的语音应用，集成了语音识别（ASR）和语音合成（TTS）功能。该项目提供了完整的语音处理解决方案，包括实时语音识别、批量音频转录、语音合成以及字幕配音生成等功能。**更新**：新增了视频导出功能，支持将音频片段和字幕合成到视频中，并增强了字幕播放器的动态视频源加载能力。
...
### 动态视频源加载组件
- **远程视频接口**：支持从外部API动态获取视频地址
- **回退机制**：当远程接口失败时自动使用默认视频源
- **实时更新**：播放器可动态替换当前视频源
...
Mobile --> Router
Player --> ExportRouter
Player --> DynamicVideoRouter
Router --> Middleware
Middleware --> Transcribe
Middleware --> TTS
Middleware --> Voiceover
Middleware --> WebSocket
Middleware --> DynamicVideo
ExportRouter --> Export
Export --> ExportRecords
Export --> ExportCache
Export --> FFmpeg
DynamicVideoRouter --> VideoSource
DynamicVideo --> WebSocket
Transcribe --> Model
TTS --> VoiceCatalog
Voiceover --> Cache
...
**描述**：返回字幕播放器页面，支持实时字幕显示、视频导出功能和动态视频源加载
...
**更新**：字幕播放器现在支持动态视频源加载，可从外部API获取视频地址并自动替换当前视频源

**动态视频源加载机制**：
- **API接口**：`http://192.168.31.145:7658/urgent/api/gamevideo`
- **响应格式**：`{"code": 200, "data": "视频URL"}`
- **回退机制**：当API调用失败时自动使用默认视频源
- **实时更新**：成功获取新视频地址后立即替换当前视频源
...
DynamicVideo --> WebSocket
DynamicVideo --> ExternalAPI[外部视频API]
```
...
DynamicVideoLoader --> Player
Demo --> Server
SpeechRecorder --> Server
Player --> ExportService
Player --> DynamicVideoLoader
```
...
### 动态视频源加载优化
- **异步加载**：视频源加载使用异步请求，不影响播放器主界面响应
- **缓存机制**：成功获取的视频地址可缓存一段时间，减少重复请求
- **错误处理**：完善的错误处理和回退机制，确保播放器稳定性
- **网络优化**：支持断线重连和超时控制
...
```bash
# 检查外部API可用性
curl -X GET http://192.168.31.145:7658/urgent/api/gamevideo
...
| 502 | Bad Gateway | 动态视频源API调用失败 | 检查外部API可用性和网络连接 |
...
- **灵活部署**：支持本地部署和云端部署
- **扩展性强**：新增的视频导出功能和动态视频源加载为项目提供了更强大的媒体处理能力
