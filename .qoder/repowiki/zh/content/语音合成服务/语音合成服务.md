...
- 本地 TTS：通过本地 Kokoro-82M 模型进行中文语音合成，支持同步和流式处理。
...
- **新增** Kokoro 本地 TTS：通过 kokoserver.py 提供本地中文语音合成服务，支持同步和流式处理。
...
- **新增** Kokoro性能：本地模型推理速度快，支持流式生成，适合中文场景；可通过语速参数平衡质量和延迟
...
## 结论
本项目提供了完整的语音合成能力：在线 DashScope TTS、Edge TTS 字幕配音、实时与离线两种播报模式，以及新增的本地 Kokoro 中文 TTS 服务。通过统一的音色目录与查询接口，前端可灵活选择音色；通过 .env 与命令行参数实现配置化管理。新增的多后端选择机制支持自动检测和手动指定，可根据环境条件选择最优的 TTS 后端。建议在生产环境中结合 FFmpeg 与 sounddevice，确保高质量与低延迟的音频处理与播放。
